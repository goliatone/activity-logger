...
@objcMembers class Keylogger {
    ...
    var sessions: [String:KeyloggerSession]
    var session: KeyloggerSession
    var timeout:Timer?
    ...
    init() {
        ...
        sessions = [:]
        session = KeyloggerSession(timer: timeout)
        ...
        //this should be updated here... how can we know which programs are more critical? We can ignore some, and give others more
        //weight...
    ...
    /**
     * Callback fired when we switch from one application to a different
     * one.
     *
     */
    @objc dynamic func activatedApp(notification: NSNotification) {
        ...
        if  let info = notification.userInfo,
            ...
            let dateFolder = CallBackFunctions.getDateFolder()
            
            //Do we have a current session for the date?
            if sessions[dateFolder] == nil {
                timeout = setTimeout(3, block: {
                    print("HELLLLOOOOO THIS IS A TIMEOUT")
                })
                session = KeyloggerSession(timer: timeout!)
                
                sessions[dateFolder] = session
            }
            
            //We want to switch to a new application.
            session.switchApplication(name: name)
            
            print("Switching to application \(name): elapsed \(session.elapsedTime())")
            ...
            let timeStamp = session.application.start.description(with: Locale.current) +  "\t\(self.appName.convertedToSlug() ?? "empty")" + "\n"
